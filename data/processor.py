...
class bertProcessor(DataProcessor): #bert_s
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            guid = "%s-%s" % (set_type, i)
            examples.append(InputExample(guid=guid, text_a=clean_text(data[i][0]), text_b=data[i][1], label=data[i][3], text_c=data[i][2]))
        return examples
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            if use_gpt:
                ...
                if args.use_template_words:    
                    ...
                    prompt = f"{SUBJECT} "+ f"{tokenizer.mask_token} "*args.multi_viewer_num + f"{OBJECT}"
                else:
